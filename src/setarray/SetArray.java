...
public class SetArray {
    ...
    public SetArray union(SetArray b) {
        ...
        int shiftB = Math.abs(b.zeroIndex - C.zeroIndex);
    ...
    public SetArray intersection(SetArray b) {
    ...
    public SetArray difference(SetArray b) {
        // Выполним проверку:
        ...
        if (this.zeroIndex >= b.zeroIndex) {
            for (int i = 0; (i + shift) < C.set.length && i < b.set.length; i++) {
                C.set[i + shift] &= ~b.set[i];
            }
        } else {
            for (int i = 0; i < C.set.length && (i + shift) < b.set.length; i++) {
                C.set[i] &= ~b.set[i + shift];
            }
        }
    ...
    public SetArray assign(SetArray b) {
        ...
        SetArray C = new SetArray(b.min, b.max);
    ...
    public boolean equal(SetArray b) {
        ...
        int shiftA = Math.abs(this.zeroIndex - b.zeroIndex);
        ...
        if (this.zeroIndex > b.zeroIndex) {
            ...
            for (i = 0; i + shiftA < this.set.length && i < b.set.length; i++)
                //Если соответствующие элементы не равны, то возвращаем false
                if (this.set[i + shiftA] != b.set[i]) return false;
            ...
            for (i = 0; i < shiftA; i++)
                //Если какой-то из элементов не равен нулю, то возвращаем false
                if (b.set[i] != 0) return false;
            //Заведём цикл, который будет длиться не закончится общий участок множеств
            for (i = 0; i < this.set.length && i + shiftA < b.set.length; i++)
                //Если соответствующие элементы не равны, то возвращаем false
                if (this.set[i] != b.set[i + shiftA]) return false;
        }
    ...
    public SetArray find(SetArray b, int x) {
        ...
        else if ((x >= b.min || x <= b.max) && (b.set[pos + b.zeroIndex] & converted_number) != 0)
